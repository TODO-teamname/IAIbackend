...
#from organizaton_mooclets.models import Mooclet, OrganizationMoocletAuthenticator
#from organizaton_mooclets.views import OrganizationMoocletViewSet
...
class MoocletViewTestCase(TestCase):
    ...
    def setUp(self):
        ...
        actions = {
            'get': 'retrieve'}
        self.view = OrganizationMoocletViewSet.as_view(actions)

        self.user = auth.get_user_model()(email="test@gmail.com", password="password", first_name="Test", last_name="User")

        self.organization = Organization(name="org 1", url=url, token=token)
